...
import { createCube, Types as CubeTypes, Faces as CubeFaces, Masks as CubeMasks } from './cube'
import { createContent, Types as ContentTypes } from './content'
...
const Scene = () => {
    ...
    useEffect(() => {
        ...
        const { regl, play } = createRenderer(containerRef.current)

        const camera = createCamera(regl)
        const cube = createCube(regl)
        const content = createContent(regl)
        const reflection = createReflection(regl, camera)
        ...
        const animate = ({ viewportWidth, viewportHeight, tick }: any) => {
            ...
            camera(cameraConfig, () => {
                ...
                // Render the mask into the maskFbo
                cube([
                    {
                        fbo: displacementFbo,
